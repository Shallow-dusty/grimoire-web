...
**问题背景**: 某些角色（如酒鬼、疯子、魔偶）玩家看到的身份与真实身份不同。
...
**问题背景**:
线下游戏时，说书人通常会将魔典投屏到电视或使用公共音响。如果此时播放"夜间行动提示音"（如小恶魔杀人反馈），所有玩家都会听到，导致信息泄露。

**解决方案**:

- **在线模式 (Online)**: 默认模式。播放所有音效，包括私密的提示音
  (Cues)。适合个人设备使用。
- **线下模式 (Offline)**: 投屏模式。自动**静音**所有 "提示音效" (Cues)
  类别，仅保留背景音 (Ambience) 和界面音 (UI)。

**操作方式**:

1. 点击说书人菜单右上角的 **齿轮图标** (Settings)
2. 在弹出的 "音频设置" 面板中切换模式
3. 也可单独开关 `Ambience` / `UI` / `Cues` 类别
4. **注意**: 在线下模式下，`Cues` 类别会被强制锁定为关闭状态
...
**扩展自定义角色**: 在 `constants.ts` 中为角色添加 `nightAction` 字段：
...
**模型选择**: 在 Controls → AI tab 下拉菜单切换模型：

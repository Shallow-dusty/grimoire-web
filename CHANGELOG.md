...
### 🎨 沉浸式哥特 UI 重构 (Gothic UI Overhaul)

本版本对游戏核心界面进行了彻底的视觉升级，营造浓厚的哥特式恐怖氛围。

- **材质升级**: 引入羊皮纸、深色石纹、磨损皮革等高清纹理。
- **字体更新**: 标题采用 `Cinzel` 字体，正文使用衬线字体，提升古典感。
- **组件重构**:
  - **Lobby**: 新增动态背景、羊皮纸风格输入框。
  - **Grimoire**: 座位、角色卡片、状态标记全面哥特化。
  - **StorytellerMenu**: 重新设计的菜单布局，更符合操作直觉。
  - **Chat**: 聊天气泡和输入框样式优化。
  - **NightActionPanel**: 夜间行动面板视觉增强。

### 🔊 音频隐私与管理 (Audio Privacy)

为了支持线下投屏和公共场合游戏，新增了音频隐私控制系统。

- **音频模式**:
  - **在线模式 (Online)**: 播放所有音效（包括秘密提示音），适合个人设备。
  - **线下模式 (Offline)**: 自动屏蔽 "提示音效"
    (Cues)，防止通过公共音响泄露夜间信息。
- **强制设置向导**: 进入游戏前强制用户选择音频环境，防止误操作。
- **实时设置**: 说书人菜单中新增音频设置入口，可随时切换模式或开关特定音效类别
  (Ambience/UI/Cues)。

### 📦 资源集成

- **音频资产**: 替换了所有占位音频，集成了高质量的音效文件。
- **图标库**: 全面使用 `lucide-react` 替换旧图标，风格更统一。
...
- **增强视觉**: 优化筹码边框颜色（存活玩家使用棕色边框
  `#b45309`，死亡玩家使用深石色 `#44403c`）
...
- **颜色修正**: 将死亡效果从红色主题 (`#dc2626`, `rgba(139,0,0)`) 改为灰烬色主题
  (`#a8a29e`, `rgba(87,83,78)`)
- **影响组件**: 闪光遮罩、脉冲波纹、骷髅图标、玩家名称、边缘渐变全部采用 Stone
  色调
...
- **修复**: `pickTownsfolk()` 现在使用 Set 收集所有已使用角色（包括 `realRoleId`
  和 `seenRoleId`）
